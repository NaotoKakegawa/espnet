...
# exp tag
tag="kana_est_data3m_len300" # tag for managing experiments.
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # non_lang_syms_...txt:<NOISE>のような非言語シンボルのリスト
    # オリジナルのやり方だと，<NOISE>以外の<.*>を非言語シンボル扱いしてしまう
    # 例：<物語>シリーズ
    # なので，<NOISE>だけを非言語シンボルとして扱うことにする
    echo "make a non-linguistic symbol list for all languages"
#    cut -f 2- -d' ' data/${train_set}.*/text.${tgt_case} | grep "<" | grep ">"| sort | uniq > ${nlsyms}
#    cut -f 2- -d' ' data/${train_set}.*/text.${tgt_case} | grep "<NOISE>"| sort | uniq > ${nlsyms}
    echo "<NOISE>" > ${nlsyms}
    cat ${nlsyms}

    echo "make a target dictionary"
    echo "<unk> 1" > ${dict_tgt} # <unk> must be 1, 0 will be used for "blank" in CTC
    text2token.py -s 1 -n 1 -l ${nlsyms} --trans_type ${trans_type} data/${train_set}.jp/text.${tgt_case} | cut -f 2- -d" " | tr " " "\n" \
    ...
    text2token.py -s 1 -n 1 -l ${nlsyms} --trans_type ${trans_type} data/${train_set}.en/text.${src_case} | cut -f 2- -d" " | tr " " "\n" \
        | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict_src}
    wc -l ${dict_src}

    echo "make json files"
    local/data2json.sh --nj 16 --text data/${train_set}.jp/text.${tgt_case} --nlsyms ${nlsyms} --trans_type ${trans_type} --skip_utt2spk true\
        data/${train_set} ${dict_tgt} > ${feat_tr_dir}/data.${src_case}_${tgt_case}.json
    local/data2json.sh --text data/${train_dev}.jp/text.${tgt_case} --nlsyms ${nlsyms} --trans_type ${trans_type} --skip_utt2spk true\
        data/${train_dev} ${dict_tgt} > ${feat_dt_dir}/data.${src_case}_${tgt_case}.json
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}; mkdir -p ${feat_recog_dir}
        local/data2json.sh --text data/${rtask}.jp/text.${tgt_case} --nlsyms ${nlsyms} --trans_type ${trans_type} --skip_utt2spk true\
    ...
    for x in ${train_set} ${train_dev} ${recog_set}; do
        ...
        data_dir=data/${x}.en
        local/update_json.sh --text ${data_dir}/text.${src_case} --nlsyms ${nlsyms} --trans_type ${trans_type} \
